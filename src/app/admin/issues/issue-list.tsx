...
const INITIAL_PER_SECTION = 15

const MORE_PER_SECTION = 10
...
export function IssueList({
  ...
  function showMore(key: string) {
    setVisibleCounts((current) => ({
      ...current,
      [key]: (current[key] ?? INITIAL_PER_SECTION) + MORE_PER_SECTION,
    }))
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        <span className="flex-1">Exporter les signalements</span>
        <ChevronRightIcon
          aria-hidden="true"
          strokeWidth={2.5}
      ...
      {reports.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {groupedReports.map((section) => {
            const shown = visibleCounts[section.key] ?? INITIAL_PER_SECTION
            const remaining = section.items.length - shown
              ...
              <section key={section.key} className="flex flex-col gap-2">
                ...
                {!collapsedSections[section.key] && (
                  <ul className="flex flex-col gap-2">
                    {section.items.slice(0, shown).map((report) => {
                ...
                {!collapsedSections[section.key] && remaining > 0 && (
                  <button
                    type="button"
                    onClick={() => showMore(section.key)}
                    className="w-full rounded-2xl border border-dashed border-canopee-forest/25 px-3 py-2.5 text-[13px] font-bold text-canopee-forest/70 transition-colors hover:border-canopee-green/50 hover:text-canopee-forest focus-visible:ring-2 focus-visible:ring-canopee-green focus-visible:outline-none"
                  >
                    Afficher plus
                    <span className="ml-1.5 font-medium text-canopee-forest/45 tabular-nums">
                      {remaining}
                    </span>
                  </button>
                )}
