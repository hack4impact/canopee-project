...
async function reportProgress(milestones) {
  ...
  const { pct, closed, open, total, byPerson, unassignedOpen } = tallyIssues(issues);
  const due = dueState(sprint, now);

  console.log(`[sprint] ${sprint.title}: ${closed}/${total} closed (${pct}%), ${due.text}`);
  ...
  await sendDiscord({
    ...
    embed: {
      ...
      url: sprint.html_url,
      color: due.overdue ? COLORS.review : pct === 100 ? COLORS.unblocked : COLORS.ready,
      description:
        `\`${progressBar(pct)}\` **${pct}%**\n\n` +
...
async function reportOverdue(milestones) {
  ...
  for (const milestone of late) {
    const issues = await listMilestoneIssues(owner, repo, milestone.number, "open");
  ...
  if (open === 0) {
    console.log("[sprint] overdue sprints have no open issues left — staying quiet");
    return;
  }
  ...
  const worst = late.slice().sort((a, b) => new Date(a.due_on) - new Date(b.due_on))[0];
  const worstDue = dueState(worst, now);

  const fields = people.slice(0, needsEveryone ? 24 : 25).map(([login, row]) => ({
  ...
  await sendDiscord({
    content:
      ...
      (late.length > 1 ? ` (and ${late.length - 1} other sprint past due)` : "") +
    ...
    pingLogins: [...people.map(([login]) => login), ...(needsEveryone ? everyone.logins : [])],
    embed: {
      title: late.length === 1 ? `Overdue sprint — ${worst.title}` : `${late.length} overdue sprints`,
      url: worst.html_url,
      color: COLORS.review,
      description:
        late.map((m) => `• **${m.title}** — ${dueState(m, now).text}`).join("\n") +
...
console.log(`[sprint:${mode}] ${milestones.length} open milestone(s) in ${repoFullName}`);
...
if (mode === "progress") {
  ...
  console.error(`::error::Unknown MODE "${mode}" — expected "progress" or "overdue".`);
  process.exit(1);
}
