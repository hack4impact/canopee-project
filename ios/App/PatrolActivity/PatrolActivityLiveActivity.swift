...
/// Reads the subset of SVG path syntax used by the pictograms in
/// `src/components/patrol-picto.tsx`: move, line, horizontal, vertical, cubic,
/// smooth cubic and close. The artwork is fitted into the frame it is handed,
...
/// The same hiker the app puts on its "Démarrer" button, ported from
/// `src/components/patrol-picto.tsx`.
...
/// The hiker in a translucent disc, the way the app frames it on the start
/// button (`PictoCircle`, `bg-white/20`).
...
/// Draws the walked route inside whatever space it is handed, keeping the
/// shape of the path. The trace is fitted on its own bounding box rather than
/// stretched to the frame, so a north-south walk stays a vertical line instead
/// of being smeared across the width.
...
/// The panel that owns the middle of the card. It is always drawn so the
/// layout never collapses: before there is a path worth showing it marks the
...
/// The one thing on the card that keeps moving on its own. A Live Activity is
/// rendered as a snapshot, so a looping SwiftUI animation would sit frozen;
/// `ProgressView(timerInterval:)` is driven by the system instead and sweeps
/// without the app being awake. It restarts from every push, so it runs from
/// the left on each new fix and only ever fills up when the patrol has gone
/// quiet.
...
/// Everything tappable carries a rim as well as a fill: on a black card a
/// translucent fill alone does not read as a control.
...
/// Four full-width bands. Every band is anchored to both edges and the hero
/// band hands its leftover width to the route panel, so the card has no gap
/// that nothing lives in.
